...
import { ArrowUpRight, CalendarDays, ClipboardCheck, Eye, Plus, Wallet } from "lucide-react";
...
export function DashboardPageClient() {
  ...
  const upcoming = stocks.items.filter((stock) => stock.nextReviewDate).sort((a, b) => (a.nextReviewDate ?? "").localeCompare(b.nextReviewDate ?? "")).slice(0, 4);
  const metrics = [["총 투자 원금", won.format(invested), "USD는 최근 매매 환율 대신 1,380원 추정"], ["현재 평가금액", won.format(marketValue), "저장된 현재가 기준"], ["미실현손익", `${marketValue - invested >= 0 ? "+" : ""}${won.format(marketValue - invested)}`, `${holdings.length}개 보유 종목`], ["계획 매매율", `${analytics.plannedTradeRate.toFixed(0)}%`, `매매 ${analytics.tradeCount}건 기준`]];
  return <><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm text-[var(--muted)]">개인용 로컬 투자 기록</p><h1 className="mt-1 text-2xl font-semibold tracking-tight">오늘의 판단을 기록해 보세요.</h1></div><Link href="/observations" className="flex items-center gap-2 rounded-lg bg-[var(--accent)] px-4 py-2.5 text-sm font-medium text-white"><Plus size={17} />새 관찰 기록</Link></div><section aria-label="포트폴리오 요약" className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{metrics.map(([label, value, note]) => <article key={label} className="rounded-xl border bg-[var(--surface)] p-5"><div className="flex items-center justify-between"><p className="text-sm text-[var(--muted)]">{label}</p><ArrowUpRight size={17} className="text-[var(--accent)]" /></div><p className="mt-4 text-2xl font-semibold tabular-nums">{value}</p><p className="mt-1 text-xs text-[var(--muted)]">{note}</p></article>)}</section><div className="mt-4 grid gap-4 lg:grid-cols-[1.45fr_1fr]"><section className="rounded-xl border bg-[var(--surface)] p-5"><div className="flex items-center justify-between"><h2 className="font-semibold">보유 자산</h2><Wallet size={19} className="text-[var(--muted)]" /></div><div className="mt-4 space-y-3">{holdings.map((stock) => { const share = marketValue ? toKrw(stock.marketValue, stock.currency) / marketValue * 100 : 0; return <div key={stock.id}><div className="mb-1.5 flex justify-between text-sm"><span>{stock.name}</span><span className="tabular-nums">{share.toFixed(1)}%</span></div><div className="h-2 rounded-full bg-[var(--surface-muted)]"><div className="h-full rounded-full bg-[var(--accent)]" style={{ width: `${share}%` }} /></div></div>; })}{!holdings.length && <p className="py-12 text-center text-sm text-[var(--muted)]">보유 종목을 등록하면 자산 구성이 표시됩니다.</p>}</div></section><section className="rounded-xl border bg-[var(--surface)] p-5"><div className="flex items-center gap-2"><CalendarDays size={19} className="text-[var(--accent)]" /><h2 className="font-semibold">다가오는 검토일</h2></div><div className="mt-4 space-y-1">{upcoming.map((stock) => <Link key={stock.id} href={`/stocks/detail?id=${stock.id}`} className="flex items-center justify-between rounded-lg px-2 py-2.5 text-sm hover:bg-[var(--surface-muted)]"><span>{stock.name}</span><span className="text-[var(--muted)]">{stock.nextReviewDate}</span></Link>)}</div></section></div><section className="mt-4 grid gap-3 sm:grid-cols-3"><Quick href="/plans" icon={<ClipboardCheck size={18} />} label="진행 중 계획" value={`${activePlans.length}개`} /><Quick href="/observations" icon={<Eye size={18} />} label="관찰 기록" value={`${observations.items.length}개`} /><Quick href="/reviews" icon={<ArrowUpRight size={18} />} label="회고" value={`${reviews.items.length}개`} /></section></>;
}
