...
export function DashboardPageClient() {
  ...
  const rateDate = exchangeRates.snapshot.rateDate
    ...
    : t("기본값");
  const metrics = [[t("총 투자 원금"), display(invested), t("원장 평균단가 · {currency} 표시", { currency: displayCurrency })], [t("현재 평가금액"), display(marketValue), t("저장된 현재가 · 환율 {date}", { date: rateDate })], [t("기록 현금"), display(cashBalanceKrw(ledger, rates)), !ledger.cashBalances.length ? t("현금 기록 없음") : ledger.cashBalances.every((item) => item.isReconciled) ? t("입금 기록 기준") : t("기초 입금 등록 필요")], [t("실현손익"), `${ledger.totalRealizedKrw >= 0 ? "+" : ""}${display(ledger.totalRealizedKrw)}`, t("거래 당시 환율 반영")], [t("미실현손익"), `${marketValue - invested >= 0 ? "+" : ""}${display(marketValue - invested)}`, t("{count}개 보유 종목", { count: formatNumber(holdings.length) })], [t("계획 매매율"), formatNumber(analytics.plannedTradeRate / 100, { style: "percent", maximumFractionDigits: 0 }), t("매매 {count}건 기준", { count: formatNumber(analytics.tradeCount) })]];

  return <><div className="dashboard-heading"><div><p>{t("장기투자 의사결정 워크스페이스")}</p><h1>{t("오늘 판단할 일을 먼저 봅니다.")}</h1></div><div className="dashboard-heading-actions"><label><span>{t("표시 통화")}</span><select aria-label={t("대시보드 표시 통화")} value={displayCurrency} onChange={(event) => void currencyPreference.setDisplayCurrency(event.target.value as Currency)}>{currencies.map((currency) => <option key={currency}>{currency}</option>)}</select></label><Link href="/observations" className="primary-action"><Plus size={17} />{t("관찰 기록")}</Link></div></div><DecisionQueue stocks={reviewsDue} plans={activePlans} />{ledgerWarningCount > 0 && <Link href="/trades" className="ledger-warning"><AlertTriangle size={18} /><span><b>{t("원장 확인이 필요한 기록 {count}건", { count: formatNumber(ledgerWarningCount) })}</b><small>{t("계산할 수 없는 기록은 현재 요약에서 제외했습니다. 매매 원장에서 확인해 주세요.")}</small></span><ArrowUpRight size={17} /></Link>}<section aria-label={t("포트폴리오 요약")} className="portfolio-metrics">{metrics.map(([label, value, note], index) => <article key={label} className={index === 1 ? "metric-primary" : undefined}><p>{label}</p><strong>{value}</strong><small>{note}</small></article>)}</section><div className="mt-4 grid gap-4 lg:grid-cols-3"><AssetAllocation holdings={holdings} total={marketValue} /><ScheduleCard title={t("다가오는 검토일")} icon={<CalendarDays size={19} />} stocks={reviewsDue} dateKey="nextReviewDate" empty={t("예정된 검토일이 없습니다.")} /><EarningsScheduleCard stocks={stocks.items} /></div><section className="dashboard-quick"><Quick href="/plans" icon={<ClipboardCheck size={18} />} label={t("진행 중 계획")} value={t("{count}개", { count: formatNumber(activePlans.length) })} /><Quick href="/observations" icon={<Eye size={18} />} label={t("관찰 기록")} value={t("{count}개", { count: formatNumber(observations.items.length) })} /><Quick href="/reviews" icon={<ArrowUpRight size={18} />} label={t("회고")} value={t("{count}개", { count: formatNumber(reviews.items.length) })} /></section><DashboardMemo /></>;
}
