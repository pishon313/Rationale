...
export function SettingsPageClient() {
  ...
  const [eodhdKeyValue, setEodhdKeyValue] = useState(""); const [hasEodhdKey, setHasEodhdKey] = useState(false);
  ...
  const [automaticBackupStatus, setAutomaticBackupStatus] = useState<AutomaticBackupStatus | null>(null);
  ...
  useEffect(() => {
    if (isTauriApp()) { void invoke<boolean>("has_api_key", { provider: "twelve-data" }).then(setHasKey); void invoke<boolean>("has_api_key", { provider: "eodhd" }).then(setHasEodhdKey); }
    ...
    if (isTauriApp()) void invoke<AutomaticBackupStatus>("get_automatic_backup_status").then(applyBackupStatus).catch(() => undefined);
  ...
  async function storeEodhdKey() { if (!isTauriApp()) return; await invoke("save_api_key", { provider: "eodhd", value: eodhdKeyValue }); setHasEodhdKey(Boolean(eodhdKeyValue)); setEodhdKeyValue(""); setMessage(t("API 키 설정을 저장했습니다.")); }
  async function openKeySite(url: string) {
    try { await openExternalUrl(url); }
    catch { setMessage(t("잠시 후 다시 시도해 주세요.")); }
  }
...
function LanguageCard() {
  ...
  return <section className="rounded-xl border bg-[var(--surface)] p-5"><div className="flex items-center gap-2"><Languages size={19} className="text-[var(--accent)]" /><h2 className="font-semibold">{t("언어")}</h2></div><p className="mt-2 text-sm leading-6 text-[var(--muted)]">{t("처음에는 Mac의 언어 설정을 따르며, 언제든 표시 언어를 직접 선택할 수 있습니다. 지원하지 않는 시스템 언어에서는 English가 사용됩니다.")}</p><label className="mt-4 block text-sm font-medium">{t("표시 언어")}<select aria-label={t("표시 언어")} value={selectedLocale ?? "system"} onChange={(event) => void setLocale(event.target.value === "system" ? null : event.target.value as Locale)} className="mt-1 h-10 w-full rounded-lg border bg-[var(--surface)] px-3"><option value="system">{t("Mac 시스템 설정")} · {languageNames[locale]}</option>{locales.map((item) => <option key={item} value={item}>{languageNames[item]}</option>)}</select></label></section>;
}
