...
import { ArchiveRestore, Coins, Download, HardDrive, KeyRound, Languages, RefreshCw, ShieldCheck, Upload, X } from "lucide-react";
...
export function SettingsPageClient() {
  ...
  async function importBackup() {
    try {
      ...
      if (!content) return;
      setPendingRestore(validateBackupPayload(JSON.parse(content)));
    } catch {
  ...
  return <>
    ...
    <div className="mt-6 grid gap-4 lg:grid-cols-2">
      ...
      <CurrencyCard />
      <section className="rounded-xl border bg-[var(--surface)] p-5"><div className="flex items-center gap-2"><HardDrive size={19} className="text-[var(--accent)]" /><h2 className="font-semibold">{t("데이터 백업")}</h2></div><p className="mt-2 text-sm leading-6 text-[var(--muted)]">{t("모든 투자 기록과 Note, 언어 설정을 하나의 JSON 파일로 저장합니다. Mac 교체 전에 백업하세요.")}</p><div className="mt-4 rounded-lg bg-[var(--surface-muted)] p-3 text-xs leading-5 text-[var(--muted)]"><p className="flex items-center gap-2 font-medium text-[var(--foreground)]"><ShieldCheck size={15} className="text-emerald-600" />{t("자동 백업")}</p><p className="mt-1">{t("Mac 앱은 하루 한 번 자동 백업하며 최근 7개를 보관합니다.")}</p><p className="mt-1">{t("최근 자동 백업")}: {automaticBackupAt ? formatBackupDate(automaticBackupAt, locale) : t("아직 없음")}</p>{automaticBackupPath && <p className="mt-1 break-all" title={automaticBackupPath}>{automaticBackupPath}</p>}</div><div className="mt-5 flex flex-wrap gap-2"><button onClick={() => void exportBackup()} className="flex items-center gap-2 rounded-lg bg-[var(--accent)] px-4 py-2 text-sm text-white"><Download size={16} />{t("전체 백업")}</button><button onClick={() => void importBackup()} className="flex items-center gap-2 rounded-lg border px-4 py-2 text-sm"><Upload size={16} />{t("백업 복원")}</button>{restoreAvailable && <button onClick={() => void undoRestore()} className="flex items-center gap-2 rounded-lg border px-4 py-2 text-sm"><ArchiveRestore size={16} />{t("마지막 복원 되돌리기")}</button>}</div></section>
