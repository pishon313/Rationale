...
import { buildPortfolioBalanceSnapshot, isBondAssetType } from "@/domain/portfolio-balance";
...
export function PortfolioAllocationPageClient() {
  ...
  const allocationStocks = useMemo(() => stockStore.allStocks.filter((stock) => !bondStockIds.has(stock.id) && !isBondAssetType(stock.assetType)), [bondStockIds, stockStore.allStocks]);
  ...
  return <div className="portfolio-allocation-workspace">
    <header className="portfolio-plan-heading allocation-page-heading">
      ...
      <div className="allocation-heading-actions">
        ...
        <button type="button" disabled={saving || !parsed || !dirty} onClick={() => void save()} className="allocation-primary-action"><Save size={16} aria-hidden="true" />{saving ? t("저장 중...") : t("Allocation 저장")}</button>
      </div>
    </header>
    ...
    <div className="allocation-dashboard-grid">
      <section aria-labelledby="allocation-target-title" className="allocation-panel allocation-asset-panel">
        ...
        <div className="allocation-table allocation-asset-table">
          <div className="allocation-table-head"><span>{t("자산군")}</span><span>{t("범위 내 위치")}</span><span>{t("현재")}</span><span>{t("목표")}</span><span>{t("차이")}</span></div>
          {categoryRows.map((row) => <div className="allocation-table-row" key={row.category}>
            <div className="allocation-asset-name"><i style={{ background: allocationColors[row.category] }} aria-hidden="true" /><b>{t(portfolioTargetAllocationCategoryName(row.category))}</b></div>
            ...
            <strong>{row.current === null ? "—" : `${formatBps(row.current)}%`}</strong>
            <AllocationPercentageInput label={t("{name} 전체 목표", { name: t(portfolioTargetAllocationCategoryName(row.category)) })} value={draft.targetInputs[row.category]} onChange={(value) => change((current) => ({ ...current, targetInputs: { ...current.targetInputs, [row.category]: value } }))} compact />
            <span className={row.drift !== null && toleranceBps !== null && Math.abs(row.drift) > toleranceBps ? "is-outside" : ""}>{row.drift === null ? "—" : signedBps(row.drift, formatNumber)}</span>
          </div>)}
        </div>
        ...
        <p className={`allocation-validation ${parsed ? "is-valid" : "is-invalid"}`}>{parsed ? t("목표 비중 합계 {value}% · 저장 가능", { value: formatBps(total) }) : t("전체 목표 또는 선택한 주식 세부 비율을 확인해 주세요.")}</p>
      </section>
    ...
    <section className="allocation-panel allocation-stock-detail" aria-labelledby="stock-detail-title">
      ...
      {!draft.stockDetailEnabled ? <div className="allocation-stock-empty"><div><b>{t("종목별 목표는 아직 사용하지 않습니다.")}</b><p>{t("현재 보유 종목을 기준으로 시작하거나 등록된 종목을 직접 추가할 수 있습니다.")}</p></div>{activeStocks.length ? <button type="button" onClick={enableStockDetails}><Plus size={16} aria-hidden="true" />{t("주식 세부 비율 설정")}</button> : <Link href="/stocks">{t("종목 먼저 등록하기")}<ArrowRight size={15} aria-hidden="true" /></Link>}</div> : <>
        <div className="allocation-table allocation-stock-table"><div className="allocation-table-head"><span>{t("종목")}</span><span>{t("범위 내 위치")}</span><span>{t("현재")}</span><span>{t("목표")}</span><span>{t("차이")}</span><span /></div>{draft.stockTargets.map((target) => {
          ...
          return <div className="allocation-table-row" key={target.id}><label className="allocation-stock-select"><span className="sr-only">{t("주식 종목")}</span><select aria-label={t("주식 종목")} value={target.stockId} onChange={(event) => change((currentDraft) => ({ ...currentDraft, stockTargets: currentDraft.stockTargets.map((item) => item.id === target.id ? { ...item, stockId: event.target.value } : item) }))}><option value="">{t("종목 선택")}</option>{stocks.filter((stock) => !stock.deletedAt || stock.id === target.stockId).map((stock) => <option key={stock.id} value={stock.id} disabled={selectedStockIds.has(stock.id) && stock.id !== target.stockId}>{stock.ticker} · {stock.name}</option>)}</select></label><AllocationRangeBar currentBps={current} targetBps={targetBps} toleranceBps={stockTolerance} color={allocationColors.stocks} /><strong>{current === null ? "—" : `${formatBps(current)}%`}</strong><AllocationPercentageInput label={t("종목 목표 비중")} value={target.weightInput} onChange={(value) => change((currentDraft) => ({ ...currentDraft, stockTargets: currentDraft.stockTargets.map((item) => item.id === target.id ? { ...item, weightInput: value } : item) }))} compact /><span className={drift !== null && stockTolerance !== null && Math.abs(drift) > stockTolerance ? "is-outside" : ""}>{drift === null ? "—" : signedBps(drift, formatNumber)}</span><button type="button" aria-label={t("{name} 세부 목표 삭제", { name: stockById.get(target.stockId)?.name ?? t("주식") })} onClick={() => change((currentDraft) => ({ ...currentDraft, stockTargets: currentDraft.stockTargets.filter((item) => item.id !== target.id), stockDetailEnabled: currentDraft.stockTargets.length > 1 }))}><Trash2 size={16} aria-hidden="true" /></button></div>;
        })}</div>
        <div className="allocation-stock-footer"><button type="button" disabled={draft.stockTargets.length >= activeStocks.length} onClick={addStockTarget}><Plus size={15} aria-hidden="true" />{t("종목 추가")}</button><p>{t("주식 세부 목표 합계")}: <b>{formatBps(draft.stockTargets.reduce((sum, target) => sum + (parsePercentageToBps(target.weightInput) ?? 0), 0))}%</b></p></div>
...
function AllocationRangeBar({ currentBps, targetBps, toleranceBps, color }: { currentBps: number | null; targetBps: number | null; toleranceBps: number | null; color: string }) {
  ...
  return <div className="allocation-range-bar" style={style} aria-hidden="true"><span className="track"><i className="permitted" /><i className="current" /><i className="target" /></span></div>;
}
