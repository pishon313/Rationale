...
export function StockTable({ stocks, accountHoldingsByStockId, onEdit, onDelete }: Props) {
  ...
  const columns = useMemo<ColumnDef<StockComputed>[]>(() => {
    ...
    return [
      ...
      { accessorKey: "currentPrice", header: t("현재가"), cell: ({ row }) => <div className="text-right"><Numeric>{money(row.original.currentPrice, row.original.currency)}</Numeric><small className="block text-[10px] text-[var(--muted)]">{priceLabel(row.original)}</small></div> },
      ...
      { accessorKey: "sector", header: t("내 분류") },
...
export function priceLabel(stock: Stock) {
  const source = stock.priceSource === "twelve-data" ? "Twelve Data" : stock.priceSource === "eodhd" ? "EODHD" : "수동 입력";
  const asOf = stock.priceQuotedAt ? ` · ${stock.priceQuotedAt.slice(0, 16).replace("T", " ")}` : "";
  if (stock.priceFreshness === "realtime") return `실시간 · ${source}${asOf}`;
  if (stock.priceFreshness === "delayed") return `${stock.priceDelayMinutes ?? "?"}분 지연 · ${source}${asOf}`;
  if (stock.priceFreshness === "eod") return `${asOf ? asOf.slice(3) : "시점 미확인"} 종가 · ${source}`;
  if (stock.priceFreshness === "unknown") return `시점 미확인 · ${source}`;
  return `수동 입력${asOf}`;
}
