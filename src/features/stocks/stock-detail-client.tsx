...
export function StockDetailClient({ stockId }: { stockId: string }) {
  ...
  const metrics = [["현재가", price(stock.currentPrice)], ["평균단가", stock.quantity ? price(stock.averagePrice) : "—"], ["보유 수량", stock.quantity.toLocaleString()], ["평가금액", price(computed.marketValue)], ["미실현손익", `${computed.unrealizedProfit >= 0 ? "+" : ""}${price(computed.unrealizedProfit)}`], ["검토할 사항", stock.reviewNote || "미설정"]];
