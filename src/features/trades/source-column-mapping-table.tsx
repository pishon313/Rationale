...
export function SourceColumnMappingTable({ assignments, onAssign }: { assignments: SourceColumnAssignment[]; onAssign: (assignment: SourceColumnAssignment, target: SourceColumnAssignmentTarget) => void }) {
  const { t } = useI18n();
    ...
    <table className="w-full min-w-[760px] text-left text-sm">
      ...
      <tbody>{assignments.map((assignment) => {
        ...
        return <tr key={key} className="border-t align-top">
          ...
          <td className="px-4 py-3"><label className="sr-only" htmlFor={`source-map-${key}`}>{t("{source} 열 매핑", { source: displayHeader })}</label><select id={`source-map-${key}`} aria-label={t("{source} 열 매핑", { source: displayHeader })} className="h-10 w-full min-w-52 rounded-lg border bg-[var(--surface)] px-3" value={assignment.target} onChange={(event) => onAssign(assignment, event.target.value as SourceColumnAssignmentTarget)}><option value="ignore">{t("가져오지 않음")}</option>{groups.map((group) => <optgroup key={group} label={t(importFieldGroupLabels[group])}>{importFieldDefinitions.filter((definition) => definition.group === group).map((definition) => { const owner = ownerByField.get(definition.field); const disabled = Boolean(owner && owner !== assignment); return <option key={definition.field} value={definition.field} disabled={disabled}>{t(definition.label)}{disabled ? ` · ${t("{source} 열에서 사용 중", { source: owner?.column.label ?? "" })}` : ""}</option>; })}</optgroup>)}</select></td>
