...
export function TradesPageClient() {
  const exchangeRates = useExchangeRates(); const currencyPreference = useCurrencyPreference();
  const display = (valueKrw: number) => formatCurrency(fromKrw(valueKrw, currencyPreference.displayCurrency, exchangeRates.snapshot.ratesToKrw), currencyPreference.displayCurrency);
  ...
  useEffect(() => {
    ...
    migrationInProgress.current = true; setIsMigrating(true);
    const initializedIds = new Set(migration.initializedStockIds); const now = new Date().toISOString();
    const migratedStocks = allStocks.map((stock) => initializedIds.has(stock.id) ? { ...stock, ledgerInitializedAt: now, updatedAt: now } : stock);
    void (async () => { try { await replaceTradesAsync(migration.trades); await replaceStocksAsync(migratedStocks); } catch { setMigrationFailed(true); setMessage("기존 보유 기록을 원장으로 옮기지 못했습니다. 앱을 다시 열어 재시도해 주세요."); } finally { migrationInProgress.current = false; setIsMigrating(false); } })();
  ...
  async function validateAndCommit(next: Trade[], changedId?: string, showInForm = false) {
    ...
    if (validationError) { if (showInForm) setFormError(validationError); else setMessage(validationError); return false; }
    try { await replaceTradesAsync(next.map(normalizeTrade)); setMessage(""); setFormError(""); setEditing(null); return true; }
  ...
  return <>
    ...
    <section className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="열린 포지션" value={`${activePositions.length}개`} note={`완료된 사이클 ${ledger.cycles.filter((item) => item.closedAt).length}개`} /><Metric label="보유 투자원금" value={display(investedKrw)} note={`거래 당시 환율 · ${currencyPreference.displayCurrency} 표시`} /><Metric label="누적 실현손익" value={`${ledger.totalRealizedKrw >= 0 ? "+" : ""}${display(ledger.totalRealizedKrw)}`} note="수수료·세금·거래 환율 반영" /><Metric label="기록 현금 환산" value={display(cashBalanceKrw(ledger, exchangeRates.snapshot.ratesToKrw))} note={`환율 ${exchangeRates.snapshot.rateDate ?? "기본값"} 기준`} /></section>
    <section className="mt-4 rounded-xl border bg-[var(--surface)] p-5"><div className="flex items-center gap-2"><WalletCards size={18} className="text-[var(--accent)]" /><h2 className="font-semibold">계좌별 현금</h2></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{ledger.cashBalances.map((item) => <div key={`${item.accountName}-${item.currency}`} className="rounded-lg bg-[var(--surface-muted)] p-3"><p className="text-xs text-[var(--muted)]">{item.accountName} · {item.currency}</p><p className="mt-1 font-semibold tabular-nums">{formatCurrency(item.balance, item.currency)}</p><p className="mt-1 text-[11px] text-[var(--muted)]">{item.isReconciled ? "입금 기록 기준" : "순현금흐름 · 조정 필요"}</p></div>)}{!ledger.cashBalances.length && <p className="text-sm text-[var(--muted)]">입출금 또는 매매 기록이 없습니다.</p>}</div></section>
    <section className="mt-4 overflow-hidden rounded-xl border bg-[var(--surface)]"><div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="bg-[var(--surface-muted)] text-xs text-[var(--muted)]"><tr>{["일시", "계좌", "종목/구분", "수량", "가격/금액", "현금 변동", "실현손익", "포지션", ""].map((head) => <th key={head} className="whitespace-nowrap px-4 py-3 font-medium">{head}</th>)}</tr></thead><tbody>{ordered.map((trade) => <TradeRow key={trade.id} trade={trade} ledger={ledger} onEdit={() => { setMessage(""); setFormError(""); setEditing(trade); }} onDelete={() => void deleteTrade(trade)} />)}</tbody></table></div>{!ordered.length && <div className="grid h-44 place-items-center text-sm text-[var(--muted)]">아직 원장 기록이 없습니다.</div>}</section>
