...
export function AccountTransferDialog({accounts,pair,onRequestCash,onClose,onSave}:{accounts:InvestmentAccount[];pair?:ReturnType<typeof getTransferPair>;onRequestCash:(accountId:string,currency:Currency,trigger:HTMLElement)=>void;onClose:()=>void;onSave:(input:AccountTransferInput)=>Promise<boolean>}) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4" role="dialog" aria-modal="true" aria-labelledby="transfer-title"><form className="w-full max-w-md rounded-xl bg-[var(--surface)] p-5" onSubmit={e=>{e.preventDefault();if(sourceMissing||targetMissing)return;setSaving(true);void onSave({sourceAccountId:source,targetAccountId:target,amount,currency,tradedAt:new Date(tradedAt).toISOString(),memo}).finally(()=>setSaving(false));}}><h2 id="transfer-title" className="text-lg font-semibold">{t(pair ? "이체 수정" : "계좌 간 이체")}</h2><label className="mt-4 block text-sm">{t("보내는 계좌")}<select className="mt-1 h-10 w-full rounded-lg border bg-[var(--surface)] px-3" value={source} onChange={e=>setSource(e.target.value)}>{active.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></label><label className="mt-3 block text-sm">{t("받는 계좌")}<select className="mt-1 h-10 w-full rounded-lg border bg-[var(--surface)] px-3" value={target} onChange={e=>setTarget(e.target.value)}>{active.filter(a=>a.id!==source).map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></label><label className="mt-3 block text-sm">{t("금액")}<input ref={amountInput} required type="number" min="0" step="any" value={amount} onChange={e=>setAmount(Number(e.target.value))} className="mt-1 h-10 w-full rounded-lg border px-3"/></label><label className="mt-3 block text-sm">{t("통화")}<select value={currency} onChange={e=>setCurrency(e.target.value as Currency)} className="mt-1 h-10 w-full rounded-lg border bg-[var(--surface)] px-3">{currencies.map(c=><option key={c}>{c}</option>)}</select></label>{(sourceMissing||targetMissing)&&<div role="alert" className="mt-3 rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm text-amber-950 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-100"><p>{t("계좌 간 이체에는 양쪽 계좌의 현재 현금이 필요합니다.")}</p><div className="mt-2 flex flex-wrap gap-2">{sourceMissing&&sourceAccount&&<button type="button" onClick={event=>onRequestCash(sourceAccount.id,currency,amountInput.current??event.currentTarget)} className="rounded-md border px-2 py-1 text-xs">{t("보내는 계좌 현재 현금 입력")}</button>}{targetMissing&&targetAccount&&<button type="button" onClick={event=>onRequestCash(targetAccount.id,currency,amountInput.current??event.currentTarget)} className="rounded-md border px-2 py-1 text-xs">{t("받는 계좌 현재 현금 입력")}</button>}</div></div>}<label className="mt-3 block text-sm">{t("일시")}<input required type="datetime-local" step="1" value={tradedAt} onChange={e=>setTradedAt(e.target.value)} className="mt-1 h-10 w-full rounded-lg border px-3"/></label><label className="mt-3 block text-sm">{t("메모")}<input value={memo} onChange={e=>setMemo(e.target.value)} className="mt-1 h-10 w-full rounded-lg border px-3"/></label><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={onClose} className="rounded-lg border px-4 py-2 text-sm">{t("취소")}</button><button disabled={saving||source===target||amount<=0||!tradedAt||sourceMissing||targetMissing} className="rounded-lg bg-[var(--accent)] px-4 py-2 text-sm text-white disabled:opacity-50">{t("이체 저장")}</button></div></form></div>;
}
