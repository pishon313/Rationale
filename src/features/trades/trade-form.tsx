    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-black/35" role="dialog" aria-modal="true" aria-labelledby="trade-form-title">
      <form className="h-full w-full max-w-2xl overflow-y-auto bg-[var(--surface)]" onSubmit={submit}>
        ...
        <div className="grid gap-5 p-5 sm:grid-cols-2">
          ...
          {isSecurity && <div className="sm:col-span-2"><Label text={t("연결된 매매 계획")}><select className={field} value={planId} onChange={(event) => setPlanId(event.target.value)}><option value="">{t("비계획 매매")}</option>{linkedPlans.map((item) => <option value={item.id} key={item.id}>{item.title}</option>)}</select></Label></div>}
