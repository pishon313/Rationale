...
export const fallbackRatesToKrw: RatesToKrw = { KRW: 1, USD: 1380, JPY: 9.2, EUR: 1600 };
...
export async function fetchLatestRates(): Promise<ExchangeRateSnapshot> {
  const response = await fetch("https://api.frankfurter.dev/v2/rates?base=KRW&quotes=USD,JPY,EUR", { cache: "no-store" });
