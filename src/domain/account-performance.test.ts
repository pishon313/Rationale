...
describe("long-term account performance", () => {
  it("현재 총자산, 순입금, 총손익과 계좌별 성과를 계산한다", () => {
    ...
    const result = buildLongTermPerformance(trades, [stock({ id: "s1", name: "A주식", currentPrice: 120 })], ledger, rates, new Date("2026-01-01"));
  ...
  it("현재가가 없는 포지션은 원가로 평가하고 표시한다", () => {
    ...
    const ledger = buildTradingLedger(trades);
    const result = buildLongTermPerformance(trades, [stock({ id: "s1", name: "미평가", currentPrice: 0 })], ledger, rates, new Date("2026-01-01"));
  ...
  it("보관된 계좌의 과거 성과를 계속 포함한다", () => {
    ...
    const result = buildLongTermPerformance(trades, [], buildTradingLedger(trades, [archived]), rates, new Date("2026-01-01"), [archived]);
