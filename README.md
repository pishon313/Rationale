...
- 전체 기록 JSON 백업, 안전한 복원 미리보기와 복원 취소
...
설정 → 전체 백업에서 JSON 파일을 저장할 수 있습니다. 복원 전에는 파일의 날짜와 항목 수를 미리 보여주며, 복원 직전의 현재 데이터도 안전 사본으로 남겨 설정에서 한 번에 되돌릴 수 있습니다.

Mac 앱은 하루에 한 번 앱 데이터 폴더에 자동 백업을 만들고 최근 7개를 보관합니다. 자동 백업의 최근 실행 시각과 위치는 설정에서 확인할 수 있습니다. 기기 고장까지 대비하려면 수동 백업 파일도 정기적으로 iCloud Drive나 외장 디스크에 보관하세요. SQLite 파일 자체를 두 Mac에서 동시에 iCloud 동기화하는 방식은 충돌 위험이 있어 지원하지 않습니다.
