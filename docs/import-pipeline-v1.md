...
The Settings Trade-ledger reset uses the same tombstone contract. Bulk-deleted imported Trades retain source identity and journal provenance. Reimporting the same file therefore produces a `previously_deleted` restoration candidate and never silently inserts a duplicate or bypasses the normal explicit-restore preflight.
